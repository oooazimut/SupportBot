...
from notifications import TaskFactory, task_status_notification
...
async def switch_off_notification(
    ...
    task: dict = task_service.get_one(callback_data.task)
    if (
        task
        ...
        task_service.update(taskid=task.get("taskid"), status=task["status"])
        await task_status_notification(**task)
