...
def create_folder_on_yandex_disk(yandex_path):
    ...
    response = requests.put(
        YANDEX_DISK_CREATE_FOLDER_URL, headers=headers, params=params
    )
    ...
    if response.status_code == 201:
        ...
        _logger.warning(
            f"Ошибка при создании папки {yandex_path}: {response.status_code}"
        )
...
def upload_to_yandex_disk(file_path, yandex_path):
    ...
    if response.status_code == 200:
        ...
        if upload_response.status_code == 201:
            ...
            _logger.warning(
                "Ошибка при загрузке файла на Яндекс.Диск:", upload_response.status_code
            )
    else:
